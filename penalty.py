...
def _lengths_to_masks(lengths, max_length):
    ...
    """
    # math_ops.range(max_length)：输出[0,1,2,...,max_length-1]
    # array_ops.expand_dims(input, 0)：在指定轴为输入数据增加一维，array[ml]->array[1][ml]
    # array_ops.shape(lengths)[0] -> batch size
    # tiled_ranges -> batch_size * max_length，[[0,1,..,ml],[0,1,...,ml]...[0,1,..,ml]]
    #
    tiled_ranges = array_ops.tile(
        array_ops.expand_dims(math_ops.range(max_length), 0),
        [array_ops.shape(lengths)[0], 1])
    lengths = array_ops.expand_dims(lengths, 1)     # [1,2,3] -> [[1], [2], [3]]
    masks = math_ops.to_float(
        math_ops.to_int64(tiled_ranges) < math_ops.to_int64(lengths))
    # 有效字符的地方为true，比各个句子length长的字符为false
    return masks
...
def _de_pad(word_embedding, sequence_length):
    ...
    # 有效字符的地方为1，比各个句子length长的字符为0
    mask = tf.cast(_lengths_to_masks(sequence_length, max_length), tf.int32)
    # 1~batch_size * max_length 排成 [batch_size, max_length] 的矩阵
    ...
    # indices有效字符的下表，如[0 1 2 5 6 7 8 10 11]
